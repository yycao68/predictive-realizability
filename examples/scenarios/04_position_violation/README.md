...
A slower, larger-excursion minimum-jerk profile (distance 1.5 rad over 2.0 s) that reaches 1.5 rad against a declared `max_position` of 1.0 rad, while velocity and acceleration limits are set generously wide. Isolates the position check (added specifically because the analyzer did not implement it until a review pass found the gap between README and code).
