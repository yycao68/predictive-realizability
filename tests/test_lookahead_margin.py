...
from realizability.lookahead_margin import lookahead_margin
...
def test_horizon_gives_real_lead_time_on_a_ramping_violation():
    ...
    result = lookahead_margin(t, ["joint1"], q, limits, horizon_s=0.3)
...
def test_zero_horizon_gives_zero_lead():
    ...
    q = np.column_stack([t**3])
    limits = {"joints": {"joint1": {"max_acceleration": 3.0}}}
    result = lookahead_margin(t, ["joint1"], q, limits, horizon_s=0.0)
...
def test_no_violation_means_no_warning_or_actual_time():
    ...
    result = lookahead_margin(t, joints, q, limits, horizon_s=0.2)
...
def test_smaller_horizon_gives_smaller_lead_time():
    ...
    q = np.column_stack([t**3])
    limits = {"joints": {"joint1": {"max_acceleration": 3.0}}}
    small = lookahead_margin(t, ["joint1"], q, limits, horizon_s=0.05)
    large = lookahead_margin(t, ["joint1"], q, limits, horizon_s=0.3)
